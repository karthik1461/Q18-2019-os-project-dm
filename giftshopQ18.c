// Here all the children are represented by processes and no of gifts that they have will be represented by their priority.
// Assuming one ms required per gift billing on counter
// that's why Bursttime[i] == Priority[i] for all i(here i represents process) 
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
...
int Priority[] ={4,5,9,7,5,3,10,4,8,2}; // Priority for processes high no will be the one with highest priority(eg:10)
...
{   bool check = check_burst(10,Burst_time);//check_burst checks all process burst time =0 returns true
    if(!check)
    {
      if(run != -1)
      { 
      ...
      int sr = search(10,Arrival_time,Ready_q_p,i); // search returns process id whose arrival time matches.
